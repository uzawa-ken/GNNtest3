...
def update_plot(fig, ax, history):
    ...
    # rel_err_val には None が混じる可能性がある
    rel_val = np.array(
        [np.nan if v is None else float(v) for v in history["rel_err_val"]],
        dtype=np.float64
    )

    # ログスケール用に 0 以下を下限 EPS_PLOT にクリップ
    ...
    # 単一軸に5本の系列を全部描画（ログスケール）
    ax.plot(epochs, loss_safe,      label="loss",               linewidth=2)
    ax.plot(epochs, data_loss_safe, label="data_loss",          linewidth=1.5)
    ax.plot(epochs, pde_loss_safe,  label="PDE_loss",           linewidth=1.5)
    ax.plot(epochs, rel_tr_safe,    label="rel_err_train(avg)", linewidth=1.5)
    ax.plot(epochs, rel_val_safe,   label="rel_err_val(avg)",   linestyle="--", linewidth=1.5)
...
def train_gnn_auto_trainval_pde_weighted(data_dir: str):
    ...
    log_path = os.path.join(OUTPUT_DIR, f"gnn_train_log_{timestamp}.txt")
    ...
    if len(history["epoch"]) > 0:
        final_plot_path = os.path.join(OUTPUT_DIR, f"training_history_{timestamp}.png")
        update_plot(fig, ax, history)  # 念のため最新状態で更新
        fig.savefig(final_plot_path, dpi=150, bbox_inches='tight')
